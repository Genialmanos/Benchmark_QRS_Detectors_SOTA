...
from algo.emd import full_emd
from algo.wavelet_coef import full_wavelets
from algo.multi_lvl import multi_lvl
from algo.KNN import KNN
from algo.KNN2 import KNN2
from algo.CNN import CNN
from algo.low_comput import low_comput
...
algorithms_list = ['Pan-Tompkins-ecg-detector', 'Hamilton-ecg-detector', 'Christov-ecg-detector',
                   ...
                   'Engelse-Zeelenberg-biosppy', 'Gamboa-biosppy', 'mne-ecg', 'heartpy', 'gqrs-wfdb', 'xqrs-wfdb', 'emd',
                   'wavelet_coef', 'multi-lvl', 'KNN', 'KNN2', 'low_comput', 'seuil_dynamique', 'clustering', 'CNN']
...
def run_algo(algorithm: str, sig: numpy.ndarray, freq_sampling: int) -> List[int]:
    ...
    if algorithm == 'Pan-Tompkins-ecg-detector':
        ...
        qrs_detections = full_emd(sig,freq_sampling)
    elif algorithm == 'wavelet_coef':
        qrs_detections = full_wavelets(sig, freq_sampling)
    elif algorithm == 'multi-lvl':
        qrs_detections = multi_lvl(sig, freq_sampling)
    elif algorithm == 'KNN':
        qrs_detections = KNN(sig, freq_sampling)
    elif algorithm == 'CNN':
        qrs_detections = CNN(sig, freq_sampling)
    elif algorithm == 'KNN2':
        qrs_detections = KNN2(sig, freq_sampling)
    elif algorithm == 'low_comput':
        qrs_detections = low_comput(sig, freq_sampling)
    elif algorithm == 'seuil_dynamique':
        qrs_detections = seuil_dynamique(sig, freq_sampling)
    elif algorithm == 'clustering':
        qrs_detections = clustering(sig, freq_sampling)
