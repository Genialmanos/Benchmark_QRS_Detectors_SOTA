...
def add_eval_global_line(performances_df: pd.DataFrame, nb_of_records: int, total_true_pos: int) -> pd.DataFrame:
    ...
    blanks = pd.DataFrame([['_____', '_____', '_____', '_____', '_____', '_____', '_____', '_____']],
                          index=['_____'],
                          columns=['nbofbeats', 'FP', 'FN', 'F', 'F(%)', 'P+(%)', 'Se(%)', 'F1(%)'])
    performances_df = performances_df._append(blanks, ignore_index=False)
    final_performances_df = performances_df._append(global_perf, ignore_index=False)
...
def main(data: str, algo: str, tol: int) -> None:
    ...
    while True:
        try:
            ...
            performances_tol = performances_tol._append(list_performance[0], ignore_index=False)
            performances_sup1 = performances_sup1._append(list_performance[1], ignore_index=False)
            performances_sup2 = performances_sup2._append(list_performance[2], ignore_index=False)
            counter += 1
